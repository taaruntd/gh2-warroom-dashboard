...
async function downloadPDF(data, filters) {
  ...
  Object.keys(grouped).forEach((cat) => {
    ...
    const tableRows = catRows.map((r) => {
      ...
      const flagIcon =
        r.priority === "Critical" ? "! " : r.priority === "Push" ? "> " : "";
      return [
        flagIcon + (r.task || ""),
        r.owner || "—",
        remarkStr || "—",
    ...
    autoTable(doc, {
      ...
      didParseCell(d) {
        if (d.section === "body") {
          const row = catRows[d.row.index];
          if (!row) return;
          if (d.column.index === 0 && row.priority === "Critical")
...
function CatTag({ cat }) {
  return (
    <span
      style={{
        background: CAT_LIGHT[cat] || "#eee",
        color: CAT_COLOR[cat] || "#333",
        fontSize: 10,
        padding: "2px 7px",
        borderRadius: 4,
        fontWeight: 600,
...
function RemarkLog({ raw, expanded }) {
  const entries = (raw || "")
    ...
    .map((line) => {
      ...
      return c === -1
        ? { date: "", text: line }
        : { date: line.slice(0, c).trim(), text: line.slice(c + 1).trim() };
    });
  if (!entries.length)
    return <span style={{ color: "#aaa", fontSize: 11 }}>—</span>;
  const show = expanded ? entries : entries.slice(-1);
  return (
      ...
      {show.map((e, i) => (
        <div
          ...
          style={{
            ...
            borderLeft:
              i === show.length - 1 ? `2px solid ${B.olive}` : "2px solid #ddd",
          }}
        >
          <span
            style={{
...
export default function App() {
  ...
  const filtered = activeData
    ...
    .sort((a, b) =>
      !sortKey
        ? 0
        : (a[sortKey] || "").localeCompare(b[sortKey] || "") * sortDir
    );
      ...
      <div style={{ maxWidth: 1440, margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            <ResponsiveContainer
              width="100%"
              height={Math.max(160, catData.length * 26 + 30)}
            >
              <BarChart
                data={catData}
                layout="vertical"
              ...
              >
                ...
                <Tooltip
                  ...
                  cursor={{ fill: "rgba(166,200,61,0.08)" }}
                />
                <Bar dataKey="value" radius={[0, 4, 4, 0]} maxBarSize={18}>
                  {catData.map((e, i) => (
                    <Cell key={i} fill={CAT_COLOR[e.name] || B.blue} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
                ...
                filtered.map((row, i) => {
                  ...
                  return (
                    <tr
                      ...
                      onMouseEnter={(e) =>
                        (e.currentTarget.style.background = B.oliveL + "88")
                      }
                      onMouseLeave={(e) =>
                      ...
                      <td
                        style={{
                          ...
                          borderLeft: `3px solid ${
                            PRI_BORDER[row.priority] || "transparent"
                          }`,
                        }}
                      >
